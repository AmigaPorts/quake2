...
struct soft_member *SOFT_AddMember(struct soft_cache *cache,unsigned long size,unsigned long id,unsigned long flags)
{
	...
	if(mem->next!=0L) mem->next->prev=mem;
	mem->prev=0L;
	cache->first=mem;
	mem->flags=flags;
	mem->data=0L;
  if(flags&SOFT_NOFLUSH) mem->data=(void *)id;
	else mem->flags|=SOFT_FREE;
	mem->size=size;
	mem->numaccesses=0;
	mem->id=id;
...
void SOFT_RemoveMember(struct soft_cache *cache,struct soft_member *member)
{
	...
	if(member->prev==0L) cache->first=member->next;
	else member->prev->next=member->next;

	if(member->next!=0L) member->next->prev=member->prev;
...
{
	while(cache->first) SOFT_RemoveMember(cache,cache->first);
...
unsigned long SOFT_ChangeMaxSize(struct soft_cache *cache,unsigned long newsize)
{
	...
	while(cache->currentsize>cache->maxsize) SOFT_FlushOldestMember(cache);
...
void SOFT_FlushOldestMember(struct soft_cache *cache)
{
	...
	while(curmem!=0L)
	{
		...
		{
			if(oldmem==0L) oldmem=curmem;
			else if(curmem->numaccesses<oldmem->numaccesses) oldmem=curmem;
		}
		curmem=curmem->next;
	}
