...
qboolean VID_CreateWindow( int width, int height, qboolean fullscreen)
{
	...
	else
	{
		...
		if (g_pScreen == NULL) return false;
	    }
	    screen_public = true;
	}
	...
	g_pWindow = OpenWindowTags(NULL,
	    ...
	    titletag,           (ULONG)"Quake 2",
	TAG_DONE);

	if (!g_pWindow) ri.Sys_Error (ERR_FATAL, "Couldn't create window");
...
void FS_SetRGB15Palette(const unsigned char *palette, qboolean swap)
{
    ...
    if (swap == true)
    {
	for (i=0; i<256; i++)  word_pal[i] = __ShortSwap(word_pal[i]);
    }
}
...
void SWimp_Shutdown( void )
{
    ...
    if (g_pScreen)
	if (screen_public == false) CloseScreen(g_pScreen);
	else UnlockPubScreen(NULL, g_pScreen);
    g_pScreen = 0;
