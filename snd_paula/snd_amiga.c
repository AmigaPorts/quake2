...
typedef struct
{
    BYTE    *m_pBufferLeft          __attribute__((packed));
    BYTE    *m_pBufferRight         __attribute__((packed));
    struct  Device *m_pTimerBase    __attribute__((packed));
    ULONG   m_nIntMask              __attribute__((packed));
    struct  timeval m_tv            __attribute__((packed));
} intdata_t;

int snd_speed = 22050;
int snd_size = 0x4000;

qboolean PAULA_Init(dma_t *dma)
{
    intdata_t *pInt;
    UBYTE alloc_mask[] = {3};
    UWORD period;
  ...
  g_pAudioIO = (struct IOAudio *)AllocVec(sizeof(struct IOAudio), MEMF_PUBLIC|MEMF_CLEAR);
    ...
    {
	TimerBase = (struct Device *)g_pTimeIO->tr_node.io_Device;
    }
    else
    {
